...
const extractVariableValue = (dom, variableName) => {
  ...
  const scriptInnerHTML = decode(episodesScript.innerHTML, {level: 'html5'})

  const startIndex = scriptInnerHTML.indexOf(variable) + variable.length
  const endIndex = scriptInnerHTML.indexOf(';', startIndex)
  return scriptInnerHTML.substring(startIndex, endIndex)
}
...
const formatAnimeList = dom => (element, i) => {
  ...
  const label = decode(element.querySelector('.Title').innerHTML, {level: 'html5'})
...
const extractAnimeGenres = dom => {
  ...
  return toArray(genresElements).map(element => decode(element.innerHTML, {level: 'html5'}))
}
